# Week Priorities

**Week of:** 2026-05-04

---

## 🎯 Top 3 This Week

The most important outcomes for this week. Everything else is secondary.

1. **Prepare for Thinktiv/Kickdrum deep dives** — Get educated on Exchange architecture details with Daniel and Mike
2. **Deliver CJ's 3 strategic documents by Thursday** — Breaking the monolith deliverables, capacity roadmap, productivity assessment
3. **Set up architectural clarity for Q2 execution** — Connect Thinktiv prep + CJ deliverables into coherent execution plan 

---

## 📋 Tasks

### Must Complete

**Monday (Before 3 PM Daniel/Mike Q&A):**
- [ ] Start Confluence reading [[04-Projects/Thinktiv_Kickdrum_Engagement_May_2026/Priority_Reading_List.md|Reading List]] - get as far as possible
  - Priority: Exchange Architecture Diagrams, Resiliency Future Plan (Gen3 scope)
  - Capture questions for 3 PM session
- [ ] Use [[04-Projects/Thinktiv_Kickdrum_Engagement_May_2026/Key_Talking_Points.md|Talking Points]] as reference

**Monday 3 PM - Daniel/Mike Architecture Q&A:**
- [ ] Ask architecture and technical detail questions for Kickdrum prep
- [ ] Platform Architecture clarifications for Wed 4:00 PM session
- [ ] Identity/SSO deep dive (for Fri 2 PM session)
- [ ] UI shared components and CoreUI overview (for Fri 2 PM session)
- [ ] Validate "breaking the monolith" scope against Resiliency Plan (for CJ)
- [ ] Get diagrams and talking points for both consultant presentations
- [ ] Note: Book follow-up Q&A later in week if needed

**Monday PM / Tuesday Morning:**
- [ ] Complete remaining Confluence reading (~3-4 hours total across Mon/Tue)
  - [ ] Exchange Architecture Diagrams (source of truth - 1 hr)
  - [ ] Resiliency Future Plan - April 2025 (Gen3 scope boundary - 30 min)
  - [ ] IPA Production Architecture docs (you're presenting - 1.5 hrs)
  - [ ] Skim: Workforce Identity Management, GHX SSO, Tenancy Model (30 min)
  - [ ] Skim: CoreUI design system docs (30 min)

**Tuesday PM (After Daniel Meeting):**
- [ ] Finalize talking points for Wed 4:00 PM session [[04-Projects/Thinktiv_Kickdrum_Engagement_May_2026/Key_Talking_Points.md|Quick Reference]]
- [ ] Review any additional materials from Daniel meeting
- [ ] Prepare diagrams or slides if needed

**Wednesday Morning (Final Review Before 4:00 PM):**
- [ ] Quick review of talking points and materials
- [ ] Run through presentation flow mentally

**Wednesday 4:00 PM - Thinktiv/Kickdrum Session 1 (90 min):**
- [ ] Present Platform Architecture (Current State)
- [ ] Capture consultant questions and feedback
- [ ] Note any insights for "breaking monolith" deliverables

**Wednesday PM / Thursday AM - CJ Deliverables:**
- [ ] Draft 3-5 specific "breaking the monolith" deliverables (use Wed session insights + Gen3 scope boundary)
- [ ] Create capacity allocation roadmap (current/Q3/Q4 pie charts)
- [ ] Prepare 4-month productivity assessment (data-backed, balanced, executive-level)
- [ ] Prep talking points for CJ meeting

**Thursday (Time TBD) - CJ Meeting:**
- [ ] Present 3 strategic deliverables to CJ (30-min review)

**Thursday PM / Friday AM:**
- [ ] Final prep for Fri 2 PM session (Identity/SSO, UI, IPA)
- [ ] Review talking points with Daniel if needed

**Friday 2:00 PM - Thinktiv/Kickdrum Session 2 (60 min):**
- [ ] Present Platform-Enabling Services (you lead 3 of 4 topics)

### Should Complete

**Team Housekeeping Rollout:**
- [ ] Present to staff meeting (5 min) - 5 simple items [[04-Projects/Team_Housekeeping_Rollout/Team_Housekeeping_Asks.md]]
  - Native Jira projects
  - Slack channels (#team-name convention)
  - Email distribution lists (team-name@ghx.com)
  - GitHub repos list + CODEOWNERS
  - Confluence team page (template provided)
- [ ] Send Slack message with template link

**Architecture Forum Launch:**
- [ ] Stage 1: Review with leadership (Daniel, Mike, Ramesh, Aaron) [[04-Projects/Architecture_Forum_Launch/Architecture_Forum_Proposal.md]]
  - Get alignment on forum purpose and format
  - Decide weekly vs bi-weekly
  - Confirm first 6 topics from backlog
- [ ] Stage 2: Review with all EMs (Daniel's, Mike's, Ramesh's, Aaron's teams)
  - Get buy-in on sending tech leads/senior engineers
  - Collect scheduling constraints
- [ ] Incorporate feedback and finalize format
- [ ] Create Confluence page with backlog link
- [ ] Send calendar invite for recurring meeting (start week of May 12) 

### If Time Permits
- [ ] 

---

## 📅 Key Meetings

| Day | Time | Meeting | Prep Needed |
|-----|------|---------|-------------|
| Mon | 8:00 AM | Exchange Incident Games 2026 | |
| Mon | 10:05 AM | Project RedMythos weekly standup | |
| Mon | 10:30 AM | Monthly Cyber Sync - Automation | |
| Mon | 2:00 PM | Daniel and Marten Annual Review chat | |
| Mon | 2:30 PM | Marten/Laura 1:1 | |
| Mon | **3:00 PM** | **Architecture Questions (Daniel & Mike)** - Kickdrum prep Q&A session | Review Confluence docs beforehand, prepare questions |
| Tue | 8:00 AM | EMS MS Portal - delivery timeline | |
| Tue | 9:05 AM | Marten Staff meeting - discuss timeline commitments to CJ | Reference CJ deliverables |
| Tue | 10:00 AM | Marten/Sandy (HR Check-in) | |
| Tue | 4:00 PM | Daniel 1x1 (weekly manager check-in) | Regular 1x1 - may touch on Kickdrum/CJ work |
| Wed | **4:00 PM** | **🎯 Thinktiv/Kickdrum Session 1: Platform Architecture (90 min)** | You + CJ, Bharat, Arshad, Greg Wilson |
| Thu | TBD | **CJ Follow-up (30 min)** - Present 3 strategic deliverables | [[04-Projects/CJ_Q2_Strategic_Deliverables/CJ_Strategic_Deliverables_Next_Week.md\|Deliverables Guide]] |
| Fri | **2:00 PM** | **🎯 Thinktiv/Kickdrum Session 2: Platform-Enabling Services (60 min)** | You present Identity/SSO, UI, IPA |

---

## 📊 Pillar Check

How does this week's work align to your strategic pillars?

| Pillar | Tasks/Focus | Balance |
|--------|-------------|---------|
| **Organizational Foundation** | Thinktiv/Kickdrum prep, Team Housekeeping, Architecture Forum | 🟩 30% — Good |
| **Breaking the Monolith** | Defining deliverables, architecture validation, forum launch | 🟩 60% — Strong |
| **AI Native SDLC** | Deferred this week | 🟥 0% — Resumes post-CJ |
| **Critical Technical Execution** | Confluence review, technical validation | 🟨 10% — Light |

**Balance Assessment:** ✅ Appropriate for Q2 — Breaking the Monolith is strategic priority

---

## ⚠️ Capacity Reality Check

**P0 Count:** 4 tasks due this week (over limit of 3)  
**Pressure Level:** High — expect 50-60 hour week  
**Risk:** One blocker could cascade deadlines

**Mitigations:**
- Monday: Confluence reading in gaps between meetings, 3 PM Daniel/Mike Q&A is key checkpoint
- Tuesday morning is critical deep work time (complete Confluence reading)
- Wednesday AM for final presentation prep after Tuesday PM finalization
- Architecture Forum can slip to week of May 12 if needed
- Book follow-up Q&A with Daniel/Mike if more questions arise

---

## 🔄 Deferred to Next Week

Tasks intentionally pushed to create focus on CJ deliverables:

- [ ] Metrics Framework definition (P2) → No hard deadline 

---

## 📝 Notes

**Thinktiv/Kickdrum Meetings Confirmed:**
- **Wednesday, May 6 at 4:00 PM** - Session 1: Platform Architecture (90 min)
  - You + CJ, Bharat, Arshad, Greg Wilson
  - Logical architecture, tech stack, integration patterns
- **Friday, May 8 at 2:00 PM** - Session 2: Platform-Enabling Services (60 min)
  - You presenting 3 of 4 topics: Identity/SSO, UI components, IPA capabilities
  - Tony M presenting payments/BI/observability

**Confluence Documentation Found (2026-05-01):**
- Rovo search returned 128 sources - excellent coverage!
- Priority reading list created with 11 must-read docs (3-4 hours)
- **Critical finding:** [Resiliency Future Plan](https://prd.hub.ghx.com/wiki/spaces/EA/pages/296812545/Resiliency+Future+Plan+-+April+2025) defines Gen3 scope boundary
  - Current work NOT redesigning EventBus or full microservices (that's Gen3)
  - This answers CJ's "how to avoid overdoing it" question
- Daniel Milburn listed as technical owner for GHX SSO (not just Mike)
- Strong documentation for all four Thinktiv/Kickdrum topics

**Week Flow:**
- **Monday:** Start Confluence reading, 3 PM Daniel/Mike Q&A (Kickdrum prep), continue reading
- **Tuesday:** Complete Confluence reading (AM), 4 PM regular 1x1 with Daniel
- **Wednesday 4:00 PM:** Thinktiv Session 1, then draft CJ deliverables
- **Thursday:** CJ meeting (present 3 deliverables)
- **Friday 2:00 PM:** Thinktiv Session 2 (you present Identity/SSO, UI, IPA)

**Context from Curtis staff meeting (2026-04-30):**
- Curtis has a spreadsheet of incoming asks from Thinktiv/Kickdrum
- Instruction: "if you have it, provide it, but don't spend time creating artifacts"
- This prep is about getting YOU educated, not creating new docs

**Week Success Definition:**

This week is successful if:
1. ✅ CJ meeting Thursday goes well — clear monolith strategy validated
2. ✅ Thinktiv/Kickdrum prep complete — ready to lead architecture discussions with confidence
3. ✅ Architecture Forum launched — first session scheduled for week of May 12

**Bonus win:** Architecture Forum first session agenda ties directly to CJ monolith deliverables (creates continuity and team buy-in)

---

## 🏁 End of Week Review

*Fill in on Friday*

### Completed
- 

### Didn't Finish
- 

### Learnings
- 

### Next Week Focus
- 
