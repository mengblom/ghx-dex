...
async function createPresentation() {
  ...
  pptx.title = 'CJ Strategic Update - Following Meeting Prep Structure';
  ...
  // Add productivity trend chart (Stories Completed only for simplicity)
  ...
  // Slide 13: Resiliency Endgame
  await html2pptx(path.join(__dirname, 'slide13.html'), pptx);

  // Slide 14: Capacity Evolution (with pie charts)
  const { slide: slide14, placeholders: p14 } = await html2pptx(path.join(__dirname, 'slide14.html'), pptx);

  // Q2 - Baseline
  slide14.addChart(pptx.charts.PIE, [{
  ...
  // Slide 15: Org Chart
  await html2pptx(path.join(__dirname, 'slide15.html'), pptx);

  // Slide 16: The Future
  await html2pptx(path.join(__dirname, 'slide16.html'), pptx);

  // Save presentation
  await pptx.writeFile({ fileName: path.join(__dirname, 'CJ-Strategic-Update-v2.pptx') });
  console.log('Presentation created: CJ-Strategic-Update-v2.pptx');
}
